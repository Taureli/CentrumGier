...
class Baza {
	...
	function pack_watch_list($idd) {
        ...
        /* Get total number of records */
        $sql = "SELECT count(id_pack) FROM pack WHERE id_user IN (SELECT kogo FROM watch WHERE kto='$idd')";
        $retval = mysql_query($sql, $conn);
        if (!$retval) {
            die('Could not get data: ' . mysql_error());
        }
        ...
        $sql = "SELECT * FROM pack WHERE id_user IN (SELECT kogo FROM watch WHERE kto='$idd') ORDER BY name ASC LIMIT $offset, $rec_limit";
